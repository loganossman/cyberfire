...
function fullDaysGen(daysInWeek){
    ...
    for(var a=0; a<daysInWeek; a++){
       // console.log("a = "+a+ "  days = "+daysInWeek)
        var element = document.createElement("div");
        obj.dt === start 
        ? (element.className="full_calander__number full_calander_number--current",element.id =month+start+year)
        : (element.className="full_calander__number",element.id =month+start+year);
        element.appendChild(document.createTextNode(start));
        document.getElementById(("div"+x)).appendChild(element);
        //if(mainDataref == "adminView"){
